...
* [cite_start]Реалізовано зв'язок Frontend -> Backend REST API -> PostgreSQL[cite: 1111, 1072].
* [cite_start]Сервер на FastAPI з реалізованими CRUD операціями[cite: 1101, 1120].

### ✅ Практична 2. Бізнес-документ і архітектура
* [cite_start]Сформовано бізнес-вимоги та Roadmap [cite: 1063-1065].
...
* [cite_start]Створено прототипи доменних моделей (Entities)[cite: 1066].
...
* [cite_start]Описано контракт API (`openapi.yaml`) та реалізовано валідацію через Pydantic[cite: 1104].

### ✅ Практична 5. Resilience & Error Handling
* **Backend Middleware:** Реалізовано Rate Limiting (429 Too Many Requests) та кореляцію запитів через `X-Request-Id`.
* **Idempotency:** Реалізовано механізм ідемпотентності для POST-запитів (через `Idempotency-Key`).
* **Smart Client:** На фронтенді реалізовано `fetchWithResilience` з автоматичними ретраями (backoff + jitter).
* **Degraded Mode:** Додано UI-режим "перевантаження" при збоях сервера.

### ✅ Практична 6. Контейнеризація (Docker)
* **Перевірка:** Успішно запущено `docker run hello-world`.
* **Dockerfiles:** Створено оптимізовані `Dockerfile` для `backend` (Python) та `sto-crm-frontend` (Node/Nginx).
* [cite_start]**Orchestration:** Налаштовано `docker-compose.yml`, що піднімає 3 сервіси (`web`, `api`, `db`) однією командою [cite: 1142-1143].
---
